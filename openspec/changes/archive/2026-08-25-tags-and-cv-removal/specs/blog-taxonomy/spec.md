...
### Requirement: Single-value post tag

**Reason**: A single tag cannot describe posts that sit at the intersection of two topics,
and the value was displayed but never linked, so it organised nothing.

**Migration**: `tag: <value>` becomes `tags: [<primary>, ...]`. The retired values map as
follows: `rag` → `[rag, search-retrieval, evaluation]`; `stacks` →
`[llm-serving, evaluation, governance]`; `infra` → `[llmops, data-engineering, rag]`.
Consumers reading `post.data.tag` read `post.data.tags[0]` where one value is needed and
`post.data.tags` where the list is appropriate.
