# content-gates Specification

## Purpose
Deterministic, model-free validation of anything this site publishes. Defines the gate
catalogue and its thresholds, the exit-code and JSON-report contract that makes a gate
callable from a hook, a script or a test without adaptation, and the golden-set
regression that keeps the thresholds calibrated against work already published.
