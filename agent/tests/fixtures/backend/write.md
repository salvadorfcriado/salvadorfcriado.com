...
4. **Prefix caching before quantisation.** It costs no accuracy, and on a workload with a long shared system prompt it usually moves first-token latency more than a smaller model would.
